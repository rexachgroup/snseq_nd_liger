# dge within haplotype classes for liger subclusters.
set.seed(0)
liblist <- c("Seurat", "tidyverse", "broom.mixed", "readxl", "batchtools", "lmerTest", "future.apply")
...
out_path_base <- "../../analysis/seurat_lchen/liger_subcluster_dge/liger_subcluster_haplotype/"
...
main <- function() {
    ...
    excludes <- filter(excludes, ct_subcluster != "calcarine-astrocyte-3") # dammit

    subj_meta <- fmt_subj_meta(in_subj_meta)
    liger_meta <- fmt_joint_meta(in_liger_meta, in_seurat_meta) %>%
        fmt_ct_subcluster %>%
    ...
    model_designs <- c("expression ~ Tau_H + pmi + age + sex + number_umi + percent_mito  + (1 | library_id)")
    subcluster_wk <- liger_meta_hap %>%
        inner_join(tibble(model_design = model_designs), by = character()) %>%
        arrange(ct_subcluster, model_design) %>%
        group_by(region, cluster_cell_type, ct_subcluster, model_design, clinical_dx) %>%
    ...
    subcluster_wk <- gather_broom(reg, subcluster_wk, "Tau_H")
    ...
    subcluster_wk <- subcluster_wk %>%
        mutate(filepath = file.path(
            out_path_base, 
            str_glue("lme_tables_dx/{region}-{cluster_cell_type}-{clinical_dx}-hap/{ct_subcluster}-hap.csv")
        ))
    
    pwalk(subcluster_wk, function(...) {
        cr <- list(...)
        dir.create(dirname(cr$filepath), recursive = TRUE, showWarnings = FALSE)
        if (!all(is.na(cr$broom_join))) {
            write_csv(cr$broom_join, cr$filepath)
        }
    })
    
}

fmt_subj_meta <- function(in_subj_meta) {
    read_xlsx(in_subj_meta) %>%
        filter(!is.na(Tau_H)) %>%
...
fmt_joint_meta <- function(in_liger_meta, in_seurat_meta) {
...
fmt_ct_subcluster <- function(liger_meta) {
    liger_meta %>%
        mutate( 
...
gather_broom <- function(reg, subcluster_wk, fmt_regex) {
    subcluster_wk %>%
        filter(job_id %in% findDone(reg = reg)$job.id) %>%
        mutate(broom_join = pmap(., function(...) {
            cr <- list(...)
            gc()
            broom_list <- loadResult(reg = reg, id = cr$job_id)
            writeLines(str_glue("{cr$ct_subcluster} {cr$job_id}"))
            if (all(is.na(broom_list))) {
                return(NA)
            }
            format_lm_output(broom_list, cr$data, fmt_regex)
        }))
}
