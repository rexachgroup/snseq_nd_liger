...
in_clusters <- "../../analysis/seurat_lchen/cluster_stability_dx/reclusters.rds"
in_batchtools <- "../../analysis/seurat_lchen/cluster_stability_dx/batchtools/"
out_jaccard <- "../../analysis/seurat_lchen/cluster_stability_dx/recluster_jaccard.csv"
in_subcluster_wk <- "../../analysis/seurat_lchen/cluster_stability_dx/subcluster_resamples.rds"
...
main <- function() {
    ...
    meta_subset <- meta %>% 
        ...
        filter((region == "insula" & cluster_cell_type == "excitatory" & clinical_dx == "bvFTD"))

    subcluster_tbs <- meta_subset %>%
        group_by(region, cluster_cell_type, clinical_dx) %>%
        group_nest(keep = TRUE)

    subcluster_tbs <- subcluster_tbs %>%
        mutate(job_id = pmap(., function(...) {
            current_row <- tibble(...)
            subcluster_wk %>% filter(region %in% current_row$region, cluster_cell_type %in% current_row$cluster_cell_type, clinical_dx %in% current_row$clinical_dx) %>% pluck("job.id")
        }))
    ...
    jaccard_max <- jacc_unnest %>%
        group_by(region, cluster_cell_type, clinical_dx, orig_cluster, job_id) %>%
