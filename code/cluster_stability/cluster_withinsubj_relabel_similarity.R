...
out_jaccard <- "../../analysis/seurat_lchen/cluster_stability_subjresample_relabel/recluster_jaccard.csv"
out_jaccard_raw <- "../../analysis/seurat_lchen/cluster_stability_subjresample_relabel/recluster_jaccard_raw.csv"
out_jaccard_plot <- "../../analysis/seurat_lchen/cluster_stability_subjresample_relabel/recluster_jaccard.pdf"
...
main <- function() {
    ...
    meta_subset <- meta %>% 
        ...
        filter((region == "insula" & cluster_cell_type == "excitatory"))
    
    meta_subset <- mutate(meta_subset, liger_clusters = map_chr(liger_clusters, function(x) {
            if(x == 2 | x == 5 | x == 6) {
                return("2+5+6")
            } else if (x == 1 | x == 12) {
                return("1+12")
