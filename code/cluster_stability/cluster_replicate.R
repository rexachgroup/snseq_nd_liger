...
in_clusters <- "../../analysis/seurat_lchen/cluster_repl/reclusters.rds"
...
calc_liger_jacc_index <- function(orig, subsample) { 
    orig_cluster_ids <- sort(unique(orig$liger_clusters))
    subs_cluster_ids <- sort(unique(subsample$liger_clusters))
    ...
    cmp_tbl <- mutate(cmp_tbl, jaccard_index = pmap_dbl(cmp_tbl, function(...) {
        ...
        int <- length(intersect(orig_cell_ids, subs_cell_ids))
        uni <- length(union(orig_cell_ids, subs_cell_ids))
        writeLines(paste(current_row$orig_cluster, length(orig_cell_ids), current_row$subs_cluster, length(subs_cell_ids), int, uni))
        return(int / uni)
    }))
    return(cmp_tbl)
}
