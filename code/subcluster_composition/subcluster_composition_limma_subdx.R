...
OUT_DIR <- "../../analysis/seurat_lchen/subcluster_composition/limma_subdx/"
...
# Break into matrix per dx + celltype.
library_cluster_mats <- library_pct %>%
    group_by(region, cell_type, clinical_dx) %>%
...
limma_mutate <- function(tb, form) {
    ...
    mutate(limma = pmap(list(data, matrix), function(data, matrix){
            ...
            limma_contrasts <- makeContrasts(contrasts = c("sexF - sexM"), levels = colnames(coef(dx_fit)))
...
limma_dx <- limma_mutate(library_cluster_mats,
    form = "~0 + sex + mean_percent_mito + pmi + median_genes") %>%
    limma_extract_coefs

summarize_limma(limma_dx)
write_limma(limma_dx, file.path(OUT_DIR, "subcluster_composition_subdx.tsv"))
saveRDS(limma_dx, file.path(OUT_DIR, "subcluster_composition_subdx.rds"))
