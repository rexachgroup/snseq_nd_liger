# Enrichment on ONLY control clusters.
set.seed(0)
liblist <- c("Seurat", "tidyverse", "batchtools", "BiocParallel", "parallel", "future.apply", "lme4", "lmerTest", "broom.mixed", "readxl")
...
## Outputs
out_path_base <- "../../analysis/seurat_lchen/liger_subcluster_enrichment_dge_ctl/"
...
main <- function() {
    ...
    # Refactor ct_subcluster labels; only use cells that match the overall Seurat cluster type.
    ...
    # Filter by control.
    liger_meta_subset <- liger_meta_subset %>%
    ...
    # Nest by subcluster; join with cluster_tbs to get metadata for background cells to test against.
    ...
    clearRegistry()
    batchExport(mget(
        c("subcluster_worker", "run_lmer_enrichment_de", "run_lmer", "prop_detected_generate")
    ))
    ...
    setJobNames(subcluster_wk$job.id, str_glue("subcluster-enrichment-ctl-{subcluster_wk$ct_subcluster}"))
    ...
    # Fetch results in both long and wide formats. Save long as one rds.
    subcluster_wk <- subcluster_wk %>%
        mutate(broom_long = pmap(., function(...) {
            cr <- list(...)
            broom_list <- loadResult(cr$job.id)
            writeLines(paste(cr$job.id))
            writeLines(str_glue("format_lm_long_output {cr$job.id}"))
    ...
    subcluster_wk <- subcluster_wk %>%
        mutate(broom_join = pmap(list(data, model_design, job.id), function(data, model_design, job.id) {
                ...
                tryCatch({
                    ...
                    writeLines(str_glue("format_lm_output {job.id}"))
...
# Batchtools workers for subcluster dge. Load and subset Seurat object by meta.
...
run_lmer_enrichment_de <- function(
    ...
    test_vars <- test_vars %>%
        mutate(
            enrichment_cluster = fct_other(liger_clusters, keep = as.character(test_cluster), other_level = "all_other_cells") %>% fct_relevel("all_other_cells")
        )
