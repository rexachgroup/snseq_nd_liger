# Damon Polioudakis
# 2019-10-11
# Run differential expression by clinical dx down-sampling number of cells
...
## Variables
script_name <- "clinical_dx_de_down_sample.R"
date <- format(Sys.Date(), "%Y%m%d")

## Outputs
out_table <- paste0(
  "../analysis/clinical_dx_de_down_sample/", date
  , "/percent_mt_8/tables/clinical_dx_de_down_sample_")
# out_graph <- paste0(
#   "../analysis/clinical_dx_de_down_sample/", date
#   , "/percent_mt_8/graphs/clinical_dx_de_down_sample_")
print(out_table)
...
main_function <- function(){
  ...
  assign("brain_region",
    ...
    envir = .GlobalEnv)
  assign("number_of_cells",
    run_de_args_tb$number_of_cells[[as.numeric(cmd_args[1])]],
    envir = .GlobalEnv)
...
run_de_with_lm_by_clinical_dx_and_cell_type <- function(
  ...
  # subset seurat object to clinical_dxs, region, cell type of interest
  # subset variables need to be global for some reason
  ...
  # randomly subset to desired number of cells
  random_subset_idx <- sample(1:ncol(seurat_obj), number_of_cells)
  seurat_obj <- seurat_obj[ ,random_subset_idx]
