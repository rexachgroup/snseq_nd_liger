...
OUT_DIR <- "../../analysis/seurat_lchen/cellbender/subcluster_dge/"
...
main <- function() {
    ...
    cluster_gene_summary <- cluster_tb %>%
        ...
        select(ct_subcluster, gene_cts)

    cluster_gene_summary %>%
        unnest(gene_cts, names_repair = "universal") %>%
        select(ct_subcluster = ct_subcluster...3, dx, up, down) %>%
...
run_lmer_de <- function(
    ...
    model <- as.formula(unique(meta_s$model_design))
...
format_lm_output <- function(
    ...
    lm_filter_out <- lm_filter_fdr %>%
        ...
        mutate(
            ...
            ct_subcluster = paste0(unique(meta$ct_subcluster))
        )
