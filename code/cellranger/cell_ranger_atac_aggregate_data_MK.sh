...
# qsub cell_ranger_atac_aggregate_data.sh [-f path to csv args file] [-o out_dir] [-i run id]
# sample run: qsub cell_ranger_atac_aggregate_data.sh -f args/cell_ranger_atac_aggregate_data_args_20191205.csv -o /u/scratch/d/dpolioud/cell_ranger_atac/aggregate/20191205 -i PFC1_at
#$ -cwd
#$ -S /bin/bash
#$ -V
#$ -N CRat_ag
...
################################################################################
echo ""
echo "Starting cell_ranger_atac_aggregate_data.sh ... "$(date)
echo ""
################################################################################
...
/u/home/d/dpolioud/bin/cellranger-atac-1.2.0/cellranger-atac aggr \
  --id=${id} \
  --csv=$TMPFILE \
  --reference=/u/project/geschwind/dhg/common_resources/ref_genomes/cellranger/refdata-cellranger-atac-GRCh38-1.2.0 \
  --normalize=none \
  --localmem=238 \
  --localcores=8
...
echo ""
echo "End of cell_ranger_atac_aggregate_data.sh ... "$(date)
################################################################################
