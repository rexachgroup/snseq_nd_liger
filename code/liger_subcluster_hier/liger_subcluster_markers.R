...
# 3. grouped marker betas as separate heatmap.
...
in_scenic_modules <- "../../resources/SCENIC_traj_list_step1.xlsx"

out_path_base <- "../../analysis/seurat_lchen/liger_subcluster_hier/heatmap_markers"
batchtools <- file.path(out_path_base, "batchtools")
options(future.globals.maxSize = Inf)
        
plot_ts <- function() {str_glue("{system('md5sum liger_subcluster_markers.R', intern = TRUE)} {date()}")}
...
read_scenic <- function(in_scenic_path) {
    sheets <- excel_sheets(in_scenic_path)
    scenic_tb <- map(sheets, function(s) {
            read_excel(in_scenic_path, sheet = s)
        }) %>% 
        setNames(nm = sheets) %>%
        bind_rows(.id = "cell_type") %>%
        select(-group) %>%
        mutate(celltype = fct_recode(celltype, "excitatory" = "ExNeuron"))
    return(scenic_tb)
}
