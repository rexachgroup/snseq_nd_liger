# MDS coords for top 100 beta variance genes per cluster in enrichment dge

set.seed(0)
liblist <- c("Seurat", "tidyverse", "readxl", "ComplexHeatmap", "circlize", "scales", "ggrepel", "patchwork")
l <- lapply(liblist, function(x) suppressPackageStartupMessages(require(x, character.only = TRUE, quietly = TRUE)))
out_path_base <- "../../analysis/seurat_lchen/liger_subcluster_hier/mds"
...
main <- function() { 
    ...
    pwalk(cluster_ct_group, function(...) {
        cr <- list(...)
        mds_path <- file.path(out_path_base, str_glue("dge_enrichment_var_beta_mds_{cr$cluster_cell_type}.pdf"))
        print(mds_path)
        
        mds_tb <- cr$beta_hclust_data$mds$points %>% 
            as.data.frame %>%
            rownames_to_column("label") %>%
            as_tibble %>%
            rename(mds_1 = V1, mds_2 = V2, mds_3 = V3, mds_4 = V4)

        pdf(mds_path, width = 30, height = 15)
        gg_1 <- ggplot(mds_tb, aes(x = mds_1, y = mds_2, label = label)) +
            geom_point() +
            geom_label_repel(max.overlaps = 20) +
            labs(title = str_glue("mds of top 100 beta variance in enrichment dge for {cr$cluster_cell_type}"))
        gg_2 <- ggplot(mds_tb, aes(x = mds_3, y = mds_4, label = label)) +
            geom_point() +
            geom_label_repel(max.overlaps = 20) +
            labs(title = str_glue("mds of top 100 beta variance in enrichment dge for {cr$cluster_cell_type}"))
        print(gg_1 + gg_2)
...
cluster_worker <- function(cluster_wk) { 
    ...
    beta_mds <- cmdscale(dist(t(dge_beta_matrix)), k = 5, add = TRUE, list. = TRUE)
    beta_heatmap <- heatmap_text_matrix(dge_plot_tb, "gene", "ct_subcluster", "beta", row_hclust = beta_hclust, legend_name = "DGE beta")
    return(list(var_genes = top_var_genes, matrix = dge_beta_matrix, hclust = beta_hclust, mds = beta_mds, heatmap = beta_heatmap))
}
