# Heatmap of control enrichment dge.
...
in_cluster_dge_wk <- "../../analysis/seurat_lchen/liger_subcluster_enrichment_dge_ctl/subcluster_wk.rds"
in_hclust <- "../../analysis/seurat_lchen/liger_subcluster_hier/hclust/dge_enrichment_var_beta_hclust.rds"
...
marker_dir <- "../../resources/markers_forPUB"

out_path_base <- "../../analysis/seurat_lchen/liger_subcluster_hier/heatmap_markers_ctl"
plot_ts <- function() {str_glue("{system('md5sum liger_subcluster_markers_ctl.R', intern = TRUE)} {date()}")}
...
main <- function() {
    ...
    cluster_ct_group <- cluster_dge_wk %>%
        ...
        inner_join(hclust, by = "cluster_cell_type")
    ...
    cluster_ct_group <- mutate(cluster_ct_group, dge_hclust = map(dge_data, mk_dge_hclust))
    ...
    pwalk(cluster_ct_group, function(...) {
        cr <- list(...)
        heatmap_path <- file.path(out_path_base, str_glue("dge_enrichment_{cr$cluster_cell_type}_hclust_group.pdf"))

        # generate caption grobs separately
        title_grob <- textGrob(
        ...
        # need a dummy graphics object to extract the heatmap's grid tree
        tmp_pdf <- function(w, h) { pdf(tempfile(), w, h) }
        pdf(tempfile())
        heatmap_gtree <- grid.grabExpr(
            draw(cr$dge_combo_heatmap),
            wrap = TRUE,
            device = tmp_pdf,
            width = unit(10, "cm"),
            height = unit(300, "cm")
        ...
        # try to guess appropriate height?
        height <- unit(max(
            (0.2 * nrow(cr$dge_marker_data)) + 20,
            0
        ), "cm")
        print(height)

        # draw heatmap with title / subtitle above.
        print(heatmap_path)
...
read_marker_tb <- function(liger_meta_in, in_marker_dir) {
    cluster_names <- unique(liger_meta_in$cluster_cell_type)
    file_names <- file.path(in_marker_dir, str_glue("plotmarkers_{cluster_names}_group.csv"))
    marker_tb <- tibble(cluster_cell_type = cluster_names, path = file_names)
    marker_tb <- marker_tb %>%
        filter(file.exists(path)) %>%
        mutate(markers = map(path, function(path) {
            read_csv(path, skip = 1, 
                col_names = c("gene", "celltype", "group"), col_types = "ccc",
                col_select = contains(c("gene", "group"))
            )
        })) %>%
        select(cluster_cell_type, markers)
    return(marker_tb)
}
...
# - Regress out variables for display purposes using negative binomial model? just scaled data for now.
...
# subset subcluster dge to scenic tfs.
mk_scenic_data <- function(liger_meta, dge_tb, dge_dx_tb, scenic_tb) {
    subclusters <- liger_meta %>% summarize(ct_subcluster = as.character(unique(ct_subcluster)))
    scenic_filter <- scenic_tb %>%
        filter(cell_type %in% liger_meta$cluster_cell_type) %>%
        filter(!duplicated(TF)) %>%
        select(TF)

    scenic_enrich_dge_tb <- inner_join(dge_tb, scenic_filter, by = c("gene" = "TF"))
    scenic_dx_dge_tb <- inner_join(dge_dx_tb, scenic_filter, by = c("gene" = "TF"))
    return(list(enrich = scenic_enrich_dge_tb, dx = scenic_dx_dge_tb))
}

# Main beta heatmap.
# Group rows by marker_tb_filter$group,
# cols by subcluster_group
mk_grouped_beta_marker_heatmap <- function(marker_data, beta_hclust, marker_tb, row_hclust = FALSE) {
    if (!is.null(marker_tb)) {
        genes <- intersect(rownames(marker_data), marker_tb$gene)
        marker_tb_filter <- filter(marker_tb, gene %in% genes)
        marker_data <- marker_data[marker_tb_filter$gene,]
        # relevel in group order
        gene_group <- factor(marker_tb_filter$group, levels = unique(marker_tb_filter$group))
...
# make annotations based on dge results for scenic tfs.
mk_scenic_annot <- function(scenic_expr_data) {
    if (nrow(scenic_expr_data$dx) > 1) {
        scenic_tstat <- pivot_matrix(scenic_expr_data$dx, "ct_subcluster", "z", c("dx", "gene"))
        print(scenic_tstat)
        height <- unit(nrow(scenic_tstat) * 10, "mm")
        colormap <- colorRamp2(
            breaks = c(min(scenic_tstat, na.rm = TRUE), 0, max(scenic_tstat, na.rm = TRUE)),
            colors = c(muted("blue"), "white", muted("red"))
        )
        heat <- Heatmap(
            scenic_tstat,
...
mk_region_colorbar <- function(meta) {
    ...
    colormap <- c("INS" = "black", "V1" = "white", "BA4" = "gray")
