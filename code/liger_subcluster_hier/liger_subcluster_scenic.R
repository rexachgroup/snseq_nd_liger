...
out_path_base <- "../../analysis/seurat_lchen/liger_subcluster_hier/heatmap_scenic"
...
main <- function() {
    ...
    cluster_ct_group <- mutate(cluster_ct_group, scenic_expr_data = pmap(list(liger_meta, dge_data, dge_dx_data), mk_scenic_data, scenic_tb))
    ...
    cluster_ct_group <- mutate(cluster_ct_group, dge_dx_annot = pmap(list(dge_dx_data, dge_data, dge_hclust, markers), mk_dge_dx_annot))
    ...
    cluster_ct_group <- mutate(cluster_ct_group, cluster_scenic_annot = map(scenic_expr_data, mk_scenic_annot))
    ...
    pwalk(cluster_ct_group, function(...) {
        ...
        plot_ts <- str_glue("{system('md5sum liger_subcluster_scenic.R', intern = TRUE)} {date()}")
        ...
        if (nrow(cr$scenic_expr_data$dx) > 1) {
            cr_row <- cr$scenic_expr_data$dx %>% 
                group_by(dx, gene) %>% 
                summarize() %>%
                nrow()
            height <- unit(0.5 * cr_row + 30, "cm")
        } else {
            height <- unit(10, "cm")
        }
...
mk_scenic_data <- function(liger_meta, dge_tb, dge_dx_tb, scenic_tb) {
    ...
    scenic_filter <- scenic_tb %>%
        filter(celltype %in% liger_meta$cluster_cell_type) %>%
        filter(!duplicated(TF)) %>%
        select(TF)
    #if (any(liger_meta$cluster_cell_type %in% c("excitatory"))) browser()
...
mk_dge_dx_annot <- function(dge_dx_data, dge_data, dge_hclust, markers) {
    dge_data <- filter(dge_data, gene %in% markers$gene)
...
mk_combo_heatmap <- function(...) {
    ...
    hmap <- hmap %v% cr$cluster_scenic_annot
